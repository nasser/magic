...
(deftest test-if
  ...
  (are [x y] (= x y)
    (if true :t) :t
    (if true :t :f) :t
    (if true :t (throw (Exception.))) :t

    (if false :t) nil
    (if false :t :f) :f
    (if false (throw (Exception.)) :f) :f

    (if nil :t) nil
    (if nil :t :f) :f
    (if nil (throw (Exception.)) :f) :f)
...
(deftest test-and
  (are [x y] (= x y)
    ...
    (and 1 true :kw nil (throw (Exception.)) 'abc "abc") nil

    (and 1 true :kw 'abc "abc" false) false
    (and 1 true :kw 'abc "abc" false (throw (Exception.))) false))
...
(deftest test-or
  (are [x y] (= x y)
    ...
    (or nil true (throw (Exception.)) false) true
    (or nil false "abc" (throw (Exception.))) "abc"))
