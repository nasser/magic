...
;; copied from clojure.tools.analyzer.passes.uniquify, patched to use gemsym
;; instead of an incrementing counter to allow for repeat analysis without
;; name collisions (nasser/magic#199)
(ns magic.analyzer.uniquify
...
(defn uniquify [name]
  (swap! *locals-frame* #(assoc-in % [name] (symbol (str name (gensym "#"))))))
